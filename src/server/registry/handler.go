...
func New(url *url.URL) http.Handler {
	...
	// handle list tag
	rootRouter.Path("/v2/{project}/{repository:.*}/tags/list").Methods(http.MethodGet).Handler(tag.NewHandler())

	// handle manifest
	// TODO maybe we should split it into several sub routers based on the method
	manifestRouter := rootRouter.Path("/v2/{project}/{repository:.*}/manifests/{reference}").Subrouter()
	manifestRouter.NewRoute().Methods(http.MethodGet, http.MethodHead, http.MethodPut, http.MethodDelete).
		Handler(manifest.NewHandler(project.Mgr, repository.Mgr, proxy))

	// handle blob
	// as we need to apply middleware to the blob requests, so create a sub router to handle the blob APIs
	blobRouter := rootRouter.PathPrefix("/v2/{project}/{repository:.*}/blobs/").Subrouter()
