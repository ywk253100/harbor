...
func (m *manifestV2ResolverTestSuite) SetupTest() {
	m.resolver = NewManifestV2Resolver()
	m.reader = &htesting.FakeBlobReader{}
}

func (m *manifestV2ResolverTestSuite) TestArtifactType() {
	m.Assert().Equal(ArtifactTypeImage, m.resolver.ArtifactType(nil))
}
...
func (m *manifestV2ResolverTestSuite) TestResolve() {
  ...
  "rootfs": {
	...
	m.reader.On("Read").Return("", []byte(config), nil)
	err := m.resolver.Resolve(nil, []byte(content), "library/hello-world", artifact, m.reader)
	m.Require().Nil(err)
	m.reader.AssertExpectations(m.T())
